...
public class RootBot extends ActivityHandler {
    ...
    protected CompletableFuture<Void> onMessageActivity(TurnContext turnContext) {
        ...
        if (text.contains("pva")
         ...
         return turnContext.sendActivity(MessageFactory.attachment(getHeroCard().toAttachment()))
        ...
        // forget skill invocation
         return activeSkillProperty.delete(turnContext).thenAccept(result -> {
            ...
            turnContext.sendActivity(MessageFactory.text(eocActivityMessage)).thenCompose(sendResult ->{
                ...
                MessageFactory.text("Back in the root Java bot.")).thenCompose(sendResult2 ->{
                    return turnContext.sendActivity(MessageFactory.attachment(getHeroCard().toAttachment()))
    ...
    private HeroCard getHeroCard() {
        CardAction nextCardAction = new CardAction();
        List<CardAction> buttons = new ArrayList<>();
        Iterator<Map.Entry<String, BotFrameworkSkill>> iterator = skillsConfig.getSkills().entrySet().iterator();

        while (iterator.hasNext()) {
            Map.Entry<String, BotFrameworkSkill> entry = iterator.next();
            nextCardAction = new CardAction();
            nextCardAction.setType(ActionTypes.MESSAGE_BACK);
            nextCardAction.setTitle(entry.getValue().getId());
            nextCardAction.setText(entry.getValue().getId());
            buttons.add(nextCardAction);
        }

        HeroCard card = new HeroCard();
        card.setTitle("Java Skill Bot Options");
        card.setText("Click one of the buttons below to initiate that skill.");
